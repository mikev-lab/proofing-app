...
// [FIX] Helper for Creep Calculation (Local copy for UI)
function calculateCreepOffset(sheetIndex, creepPerSheetPoints) {
    return sheetIndex * creepPerSheetPoints;
}
...
async function renderSheetOnCanvas(ctx, sheetWidth, sheetHeight, sheetIndex, side, projectData) {
    ...
    const creepPoints = (currentSettings.creepInches || 0) * INCH_TO_POINTS; // [FIX] Read Creep
    ...
    const currentColumns = currentSettings.impositionType === 'booklet' ? 2 : currentSettings.columns;
    const totalRequiredWidth = (artBoxWidth * currentColumns) + (Math.max(0, currentColumns - 1) * (currentSettings.horizontalGutterInches * INCH_TO_POINTS));
    ...
    for (let row = 0; row < currentSettings.rows; row++) {
        for (let col = 0; col < currentColumns; col++) {
            const slotIndex = row * currentColumns + col;
            const pageNum = pagesOnThisSide[slotIndex];
            if (!pageNum) continue;

            const page = await pdfDoc.getPage(pageNum);
            
            let x = startX + col * (artBoxWidth + (currentSettings.horizontalGutterInches * INCH_TO_POINTS));
            const y = startY + row * (artBoxHeight + (currentSettings.verticalGutterInches * INCH_TO_POINTS));

            // --- [FIX] CREEP PREVIEW ---
            // sheetIndex 0 = Outer. sheetIndex MAX = Inner.
            if (currentSettings.impositionType === 'booklet' && creepPoints !== 0) {
                const creepOffset = calculateCreepOffset(sheetIndex, creepPoints);
                if (col === 0) {
                     // Left side -> Move Right (Towards Spine)
                    x += creepOffset;
                } else {
                     // Right side -> Move Left (Towards Spine)
                    x -= creepOffset;
                }
            }
