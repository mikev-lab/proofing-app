...
async function renderSheetOnCanvas(ctx, sheetWidth, sheetHeight, sheetIndex, side, projectData) {
    ...
    if (currentSettings.source === 'cover' && interiorPdfDoc) {
        ...
        // The PDF Viewport is the MediaBox (Wrap + Bleed).
        // To get Trim, we subtract the configured bleed.
        // This ensures crop marks start at the Trim line (inside the PDF), not outside it.
        trimWidth = view.width - (2 * bleedPoints);
        trimHeight = view.height - (2 * bleedPoints);
    } else {
