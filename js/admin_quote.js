...
function renderWeights(type, container, isCover = false) {
    container.innerHTML = '';
    const finish = state.selections[isCover ? 'coverFinish' : 'paperFinish'];
    if (!finish) {
    ...
    // Filter Logic
    // If 'Uncoated', look for type='Uncoated'
    // If 'Matte'/'Gloss', look for type='Coated' AND finish=Matte/Gloss
    // Note: Inventory data has 'type' (Uncoated, Coated) and 'finish' (Uncoated, Matte, Gloss, etc.)

    const relevantPapers = state.paperList.filter(p => {
        // Broad filter first
        const isCoverStock = p.name.toLowerCase().includes('cover') || p.gsm > 200; // heuristic
        if (isCover && !isCoverStock) return false;
        if (!isCover && isCoverStock) return false;

        if (finish === 'Uncoated') return p.type === 'Uncoated';
        if (finish === 'Matte') return p.type === 'Coated' && (p.finish === 'Matte' || p.finish === 'Silk');
        if (finish === 'Gloss') return p.type === 'Coated' && p.finish === 'Gloss';
        return false;
    });
    ...
    relevantPapers.forEach(paper => {
        ...
        btn.textContent = paper.name; // e.g. "100lb Gloss Text"
        btn.addEventListener('click', () => {
            // Deselect siblings
