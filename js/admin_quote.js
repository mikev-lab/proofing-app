...
async function calculateQuote() {
    ...
    try {
        ...
        // Display
        // Note: For admins, backend returns 'totalCost' (full breakdown). For clients, 'totalPrice' (sanitized).
        // Since this is admin_quote.js, we expect 'totalCost', but we check both for robustness.
        const total = data.totalCost !== undefined ? data.totalCost : data.totalPrice;

        document.getElementById('final-price-display').textContent = (total || 0).toLocaleString('en-US', { style: 'currency', currency: 'USD' });
        document.getElementById('price-per-unit-display').textContent = (data.pricePerUnit || 0).toLocaleString('en-US', { style: 'currency', currency: 'USD' }) + ' / unit';
        document.getElementById('shipping-display').textContent = (data.shippingCost || 0) > 0 ? `+ ${(data.shippingCost || 0).toLocaleString('en-US', {style:'currency', currency:'USD'})} Shipping` : 'Free Shipping / Pickup';
