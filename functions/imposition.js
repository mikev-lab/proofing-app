...
async function imposePdfLogic(params) {
    ...
    if (includeCover && coverFilePath && fs.existsSync(coverFilePath)) {
        try {
            ...
            // [FIX] Save and reload to ensure all pages (including inserted blank ones) have valid content streams for embedding
            const mergedBytes = await mergedDoc.save();
            inputPdfDoc = await PDFDocument.load(mergedBytes);
