...
4.  Change the **Build Command** to:
    ```bash
    npm install && npm run build && npx medusa-admin build
    ```
    *(Note: Depending on your template, `npx medusa-admin build` might be `npm run build:admin`. Check your `package.json`.)*
5.  Redeploy.
