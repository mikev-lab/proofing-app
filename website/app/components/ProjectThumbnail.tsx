...
export default function ProjectThumbnail({ url }: { url: string }) {
    ...
    return (
        <div className="w-full h-full overflow-hidden relative bg-white flex items-center justify-center">
            <Document file={url} loading={<div className="w-full h-full bg-slate-800 animate-pulse"></div>} error={<div className="text-xs text-red-400 p-2">Failed to load preview</div>}>
                <Page
                    pageNumber={1}
                    width={300}
