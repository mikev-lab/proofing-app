...
const Thumbnail = ({ pdf, pageIndex, viewMode, isCurrent, onClick, rtl, bleedInches = 0.125 }: any) => {
        ...
        const renderThumb = async () => {
            try {
                ...
                for (let i = 0; i < pageProxies.length; i++) {
                    ...
                    ctx.fillRect(currentX, 0, numSize, numSize);

                    ctx.fillStyle = 'white';
                    ctx.font = 'bold 12px sans-serif';
                    ctx.textAlign = 'center';
                    ctx.textBaseline = 'middle';
                    ctx.fillText(pages[i].toString(), currentX + numSize/2, numSize/2);
    ...
    return (
        ...
        >
            <div className="flex items-center justify-center overflow-hidden rounded shadow-sm border border-slate-700 w-full">
              ...
              {pageIndices.map((pageNum, idx) => {
                  ...
                  if (isSpread) {
                      if (idx === 0) {
                           ...
                           // Using clip-path effectively crops the visual
                           clipStyle.clipPath = `inset(0px ${bleedPx}px 0px 0px)`;
                           // Use negative margin to pull the next element left, covering the gap
                           clipStyle.marginRight = `-${bleedPx}px`;
                      } else {
