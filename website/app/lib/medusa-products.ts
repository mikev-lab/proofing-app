...
export async function getProductByHandle(handle: string): Promise<ProductData | null> {
    ...
    if (isMedusaConfigured()) {
        try {
            ...
            // NOTE: 'fields' selects which fields to return, 'expand' includes relations.
            // We need 'categories' and 'variants' expanded.
            const { products } = await sdk.store.product.list({
                handle: handle,
                limit: 1,
                fields: "id,title,handle,description,metadata,options,type_id,collection_id",
                expand: "categories,variants,options,type"
            });
