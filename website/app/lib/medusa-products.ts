...
export async function getAllProductHandles(): Promise<{ slug: string }[]> {
    ...
    if (isMedusaConfigured()) {
        try {
            const { products } = await sdk.admin.product.list({ limit: 100, fields: 'handle' });
    ...
    if (isMedusaConfigured()) {
        try {
            const { products } = await sdk.admin.product.list({
                handle: handle,
                limit: 1,
                // Expand relations needed for mapping
                // @ts-ignore - SDK types might be strict, but these are valid query params
                expand: 'categories,options,options.values,collection,tags,type'
            });
