...
export async function getProductByHandle(handle: string): Promise<ProductData | null> {
    ...
    if (isMedusaConfigured()) {
        try {
            ...
            // Removing 'expand' as it causes 400 Bad Request.
            // Requesting fields including relations should trigger implicit expansion or return the objects if supported.
            // We also add +categories, +type, +variants, +options to ensuring they are fetched if the SDK parses '+' correctly,
            // or just list them.
            const { products } = await sdk.store.product.list({
                handle: handle,
                limit: 1,
                fields: "id,title,handle,description,metadata,options,variants,categories,type,collection_id,type_id"
            });
