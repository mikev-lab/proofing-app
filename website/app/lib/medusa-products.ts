...
function mapMedusaProduct(product: any): ProductData {
    ...
    // DEBUG: Print metadata to verify format
    if (typeof window === 'undefined') { // Server-side log
        console.log(`[Medusa Debug] Mapping product: ${product.handle}`);
        console.log(`[Medusa Debug] Product Keys:`, Object.keys(product));
        console.log(`[Medusa Debug] Explicit product.metadata:`, product.metadata);
        console.log(`[Medusa Debug] Explicit product.options:`, JSON.stringify(product.options, null, 2));
        console.log(`[Medusa Debug] Processed metadata:`, JSON.stringify(metadata, null, 2));
    }
        ...
        if (Array.isArray(metadata.features)) {
            ...
            try {
                ...
                if (Array.isArray(parsed)) {
                    features = parsed;
                    if (typeof window === 'undefined') console.log(`[Medusa Debug] Successfully parsed JSON 1:`, features);
                } else {
...
export async function getProductByHandle(handle: string): Promise<ProductData | null> {
    ...
    if (isMedusaConfigured()) {
        try {
            ...
            // Explicitly request metadata and other essential fields
            const { products } = await sdk.store.product.list({
                handle: handle,
                limit: 1,
                fields: "id,title,handle,description,metadata,options,variants,categories,type"
            });
