...
export default function AdminOrders() {
  ...
  useEffect(() => {
      const fetchOrders = async () => {
          try {
              const { orders } = await medusaAdmin.admin.orders.list();
              if (orders) {
                  setOrders(orders.map(o => ({
                      ...
                      rawId: o.id // For linking to details
                  })));
              }
          } catch (e) {
              console.log("Admin: Using mock orders.");
          }
      };
      fetchOrders();
  }, []);
                    ...
                    {orders.map((order) => (
                        <tr key={order.id} className="hover:bg-slate-700/50 transition-colors">
                            <td className="px-6 py-4 font-medium text-white">
                                <Link href={`/admin/orders/${order.rawId || order.id}`} className="hover:text-indigo-400">#{order.id}</Link>
                            </td>
                            ...
                            <td className="px-6 py-4 text-right">
                                <Link href={`/admin/orders/${order.rawId || order.id}`} className="text-indigo-400 hover:text-indigo-300 hover:underline">View</Link>
                            </td>
                        </tr>
                    ))}
                </tbody>
