...
export default function ClientDashboard() {
  ...
  const openPlaceOrder = (project: any) => {
            ...
            <div className="space-y-12">
                ...
                <section>
                    ...
                    {projects.length === 0 ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {projects.map(proj => (
                                <div key={proj.id} className="bg-slate-800 border border-slate-700 rounded-xl overflow-hidden hover:border-indigo-500/50 transition-all shadow-md group flex flex-col">
                                    <div className="p-5 flex-1">
                                        <div className="flex justify-between items-start mb-2">
                                            <h3 className="text-lg font-semibold text-white group-hover:text-indigo-300 transition-colors truncate pr-2" title={proj.projectName}>
                                        ...
                                        <div className="flex items-center text-sm text-gray-400 mb-4">
                                            ...
                                            {proj.createdAt?.seconds ? new Date(proj.createdAt.seconds * 1000).toLocaleDateString() : 'N/A'}
                                        </div>
                                        {/* Specs Summary (Optional) */}
                                        {proj.specs && (
                                            <div className="text-xs text-gray-500 space-y-1">
                                                <p>Size: {proj.specs.dimensions ? `${proj.specs.dimensions.width}x${proj.specs.dimensions.height}` : 'N/A'}</p>
                                                <p>Qty: {proj.specs.quantity || 0}</p>
                                            </div>
                                        )}
                                    </div>
                                    ...
                                    <div className="bg-slate-900/50 p-4 border-t border-slate-700 grid grid-cols-2 gap-3">
                                        <button
                                            ...
                                            className="flex items-center justify-center px-3 py-2 bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-medium rounded transition-colors"
                                        >
                                            Place Order
                                        </button>
                                        <Link
                                            href={`/proof?id=${proj.id}`}
                                            className="flex items-center justify-center px-3 py-2 bg-slate-700 hover:bg-slate-600 text-white text-sm font-medium rounded transition-colors"
                                        >
                                            View Proof
                                        </Link>
                                    </div>
                ...
                <section>
                    <div className="flex justify-between items-center mb-6">
                        ...
                        <Link href="/dashboard/orders" className="text-sm text-indigo-400 hover:text-indigo-300">View All Orders &rarr;</Link>
                    </div>
                    ...
                    <div className="bg-slate-800 border border-slate-700 rounded-xl overflow-hidden">
                        {orders.length === 0 ? (
                            ...
                            <div className="divide-y divide-slate-700">
                                {orders.map(order => (
                                    <div key={order.id} className="p-4 hover:bg-slate-700/30 transition-colors flex justify-between items-center">
                                        <div>
                                            <div className="text-sm font-medium text-white mb-1">Order #{order.display_id}</div>
                                            <div className="text-xs text-gray-400">{new Date(order.created_at).toLocaleDateString()}</div>
                                        </div>
                                        <div className="flex items-center gap-6">
                                            <span className={`text-xs px-2 py-1 rounded-full ${
                                                order.payment_status === 'captured' ? 'bg-green-900/30 text-green-400' :
                                                order.payment_status === 'awaiting' ? 'bg-yellow-900/30 text-yellow-400' : 'bg-gray-700 text-gray-300'
                                            }`}>
                                                {order.payment_status}
                                            </span>
                                            <span className="text-sm font-bold text-white w-20 text-right">${(order.total / 100).toFixed(2)}</span>
                                        </div>
                                    </div>
                                ))}
                            </div>
